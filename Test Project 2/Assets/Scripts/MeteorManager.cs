...
public class MeteorManager //: MonoBehaviour
{
    ...
    {
        Transform obj = GameObject.Instantiate(meteoObj, new Vector3(Random.Range(-xRangeRandom, xRangeRandom), 5, 0),
            Quaternion.identity);
    ...
    //     Transform obj = Instantiate(meteoObj, new Vector3(Random.Range(-xRangeRandom, xRangeRandom), 5, 0), Quaternion.identity);
    //     meteoInfoList.Add(new MeteoInfo(obj));
    ...
    {
        Transform obj = GameObject.Instantiate(enemyObj, new Vector3(Random.Range(-xRangeRandom, xRangeRandom), 5, 0), Quaternion.identity);
    ...
    public void ProcessCollision(int id)
    {
        ...
        if (enemyInfoDict.TryGetValue(id, out enemyInfo)&&!isInteracted)
        {
            isInteracted = true;
    ...
    public void OnBullerHit(int id)
    {
        EnemyInfo enemyInfo;
        if (enemyInfoDict.TryGetValue(id, out enemyInfo))
        {
            ...
            AllManager.GetInstance().score++;
            UIManager.GetInstance().changeScoreEvent?.Invoke(this, AllManager.GetInstance().score);
        }
    }
}
