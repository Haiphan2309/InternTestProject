...
public class ItemManager// : MonoBehaviour
{
    ...
    public void MyUpdate()
    {
        ...
        for (int i = 0; i < itemInfoList.Count; i++)
        {
            if (itemInfoList[i].itemObj.position.y >= 6)
            {
    ...
    public void SpawnItem()
    {
        ...
        if (rand == 0)
        {
            obj = GameObject.Instantiate(itemBeamConfig._itemPrefab, new Vector3(Random.Range(-5, 5), 5, 0), Quaternion.identity);
        ...
        {
            obj = GameObject.Instantiate(itemBurstConfig._itemPrefab,  new Vector3(Random.Range(-5, 5), 5, 0), Quaternion.identity);
